...
var imageSystem = function() {
    ...
    this.uploadDutyPhoto = function(req, res, next) {
        var form = new formidable.IncomingForm();
        form.encoding = "utf-8";
        form.uploadDir = path.join(__dirname, './../upload/duty');
        form.maxFileSize = 10 * 1024 * 1024;
        form.parse(req, function(err, fields, files) {
            try {
                if(err){
                ...
                var oldpath = files["file"].path;
                var newpath = path.join(path.dirname(oldpath), req.body.did + path.extname(files["file"].name));
                fs.rename(oldpath, newpath, (err)=>{
                    try {
                        if(err) throw err;
                        res.send({"msg" : "success"});
    ...
    this.getDutyPhoto = function(req, res, next) {
        let photo = path.join(__dirname, './../upload/duty/' + req.params.id);
        fs.exists(photo + ".png", function(isExist){
            if(isExist){
                res.sendFile(photo + ".png");
                return;
                ...
                res.sendFile(path.join(__dirname, './../upload/duty/default.png'));
                return;
            }
        });
    };
